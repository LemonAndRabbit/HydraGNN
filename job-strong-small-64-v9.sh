...
for NN in 16; do
	...
	srun -N$NN -n$((NN*8)) -c7 --gpus-per-task=1 --gpu-bind=closest \
		python -u ./examples/multidataset/train.py --multi_model_list="OC2022" --multi --num_epoch=3 \
		--everyone --ddstore --log=exp-strongfull-$MS-$SLURM_JOB_ID-NN$NN-None-Deepspeed --inputfile=${MS}_MTL.json
	sleep 5
	for DS_STAGE in 3; do
		srun -N$NN -n$((NN*8)) -c7 --gres=gpu:8 \
			python -u ./examples/multidataset/train_deepspeed.py --multi_model_list="OC2022" --multi --num_epoch=3 \
			--everyone --ddstore --log=exp-strongfull-$MS-$SLURM_JOB_ID-NN$NN-ZERO${DS_STAGE}-ALL-VISIBLE --inputfile=${MS}_MTL_zero${DS_STAGE}.json \
			--use_deepspeed
		sleep 5
	done
done
