...
from hydragnn.utils.distributed import get_device, get_last_device, print_peak_memory, check_remaining
...
def train_validate_test_model_parallel(
    model,
    optimizer,
    train_loader,
    val_loader,
    ...
    if create_plots and plot_init_solution:  # visualizing of initial conditions
        _, _, true_values, predicted_values = test_model_parallel(test_loader, model, verbosity)
    ...
    timer = Timer("train_validate_test_model_parallel")
    timer.start()
    ...
    for epoch in range(0, num_epoch):
        ...
        with profiler as prof:
            tr.enable()
            tr.start("train_model_parallel")
            train_loss, train_taskserr = train_model_parallel(
                train_loader, model, optimizer, verbosity, profiler=prof
            )
            tr.stop("train_model_parallel")
            tr.disable()
            if epoch == 0:
                tr.reset()
        ...
        val_loss, val_taskserr = validate_model_parallel(
            val_loader, model, verbosity, reduce_ranks=True
        )
        test_loss, test_taskserr, true_values, predicted_values = test_model_parallel(
            test_loader,
            model,
            verbosity,
            reduce_ranks=True,
...
def get_head_indices(model, data):
    ...
    if all(ele == "graph" for ele in model.head_type):
...
def train_model_parallel(
    ...
    total_error = torch.tensor(0.0, device=get_last_device())
    tasks_error = torch.zeros(model.num_heads, device=get_last_device())
    num_samples_local = 0
    model.train()
