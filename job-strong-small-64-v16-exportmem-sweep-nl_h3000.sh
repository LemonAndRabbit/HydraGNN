...
# for MS in LARGE_H3000 LARGE_H4000 LARGE_H5000 LARGE_H6000 LARGE_H7000; do
# for NN in 16; do
...
for MS in LARGE_H3000_L12; do
for NN in 16; do
	
	srun -N$NN -n$((NN*8)) -c7 --gres=gpu:8 \
		python -u ./examples/multidataset/train_deepspeed_exportmem_allowcheckpoint_zs.py --multi_model_list="OC2022" --multi --num_epoch=1 \
		--everyone --ddstore --log=exp-strongfull-$MS-$SLURM_JOB_ID-NN$NN-CHECKPOINT --inputfile=${MS}_MTL.json \
		--use_deepspeed --conv_checkpoint
	done
done

for MS in LARGE_H3000; do
for NN in 16; do
	
	srun -N$NN -n$((NN*8)) -c7 --gres=gpu:8 \
		python -u ./examples/multidataset/train_deepspeed_exportmem_allowcheckpoint_zs.py --multi_model_list="OC2022" --multi --num_epoch=1 \
		--everyone --ddstore --log=exp-strongfull-$MS-$SLURM_JOB_ID-NN$NN-CHECKPOINT-ZS1 --inputfile=${MS}_MTL.json \
		--use_deepspeed --conv_checkpoint --zero1
	done
done

for MS in LARGE_H3000_L18 LARGE_H3000_L24 LARGE_H3000_L30; do
for NN in 16; do
...
for MS in LARGE_H3000_L12 LARGE_H3000_L18 LARGE_H3000_L24 LARGE_H3000_L30; do
for NN in 16; do
